function [y] = dsincp(x,N)
%
% function [y] = sincp(x,N)
% compute derivative of periodic sinc (used in jitterfunc)
%
...
% resulting work. 
y = (2*pi*sin(2*pi*x).*(1 - cos(2*pi*x/N)) - 2*pi*sin(2*pi*x/N).*(1 - cos(2*pi*x))/N)./(1 - cos(2*pi*x/N)).^2;
y = (y ./ sincp(x,N)) /(2*N^2);
y(mod(x,1) == 0) = 0;
return