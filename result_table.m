 ...
 for ir = 1:length(Ot_psd)
       ...
       %tic
       [v_piva, Q, k1, k2, rsz_piva] = getJRS_priori(image_Y(1:512,1:512), resize_fact);
      % toc 
 ...
 TP = zeros(1,length(resize_fact));
 
 for  a = 1:length(rst_tab)
     if(rst_tab(a,7)==rst_tab(a,14))
        % rst_tab(a,7)
         TP(1,mapObj(rst_tab(a,7))) = TP(1,mapObj(rst_tab(a,7)))+1;
     end
 end
 
 
 
 TP_P = zeros(1,length(resize_fact));
 for  a = 1:length(rst_tab)
     if(rst_tab(a,7)==rst_tab(a,16))
        % rst_tab(a,7)
         TP_P(1,mapObj(rst_tab(a,7))) = TP_P(1,mapObj(rst_tab(a,7)))+1;
     end
 end
...
%er_psd = 0;
%er_piva = 0;
%  for io = 1:length(rst_tab)
%      
%        %% Error of PIVA Method %% 
%      if(rst_tab(io,2)==rst_tab(io,14))
%          
%      
%          er_piva = er_piva+1;
%      end
